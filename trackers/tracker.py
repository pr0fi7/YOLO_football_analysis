...
class Tracker:
    ...
    def interpolate_ball_positions(self, ball_positions):
        ...
        bbox_values = [list(item.values())[0]['bbox'] if item else [None, None, None, None] for item in ball_positions]

        # Create DataFrame
        df_ball_positions = pd.DataFrame(bbox_values, columns=['x1', 'y1', 'x2', 'y2'])
    ...
    def draw_triangle(self, frame, bbox, color):
        ...
        triange_points = np.array([[x, y], [x - 10, y - 20], [x + 10, y - 20]], np.int32)
        cv2.drawContours(frame, [triange_points], 0, color, -1)
        cv2.drawContours(frame, [triange_points], 0, (0,0,0), 2)
        return frame
